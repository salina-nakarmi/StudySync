...
// Repository tab is only shown when the project has GitHub integration enabled.
...
const REPOSITORY_NAV_ITEM = { id: "repository", label: "Repository", icon: GitBranchIcon };

export default function ProjectSidebar({ project, activeTab, onTabChange, onInvite }) {
  // project.is_github_integrated drives whether the Repository tab appears.
  // If the project data isn't loaded yet, default to hiding it.
  const navItems = project?.is_github_integrated
    ? [...BASE_NAV_ITEMS, REPOSITORY_NAV_ITEM]
    : BASE_NAV_ITEMS;
      ...
      <div className="px-5 py-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#2C76BA] flex items-center justify-center text-white font-bold text-sm shrink-0">
            {project?.icon ?? project?.name?.[0]?.toUpperCase() ?? "P"}
          </div>

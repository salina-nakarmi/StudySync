...
const ARCHIVED_PROJECTS = [
  { id: 99, name: "Database Systems", subtitle: "Completed 3 mos ago" },
];
...
export default function Projects() {
  ...
  const handleCreate = ({ name, invites, githubEnabled }) => {
    setProjects((prev) => [
      ...prev,
      {
        id: Date.now(),
        name,
        subtitle: `Solo Project${invites.length ? ` • ${invites.length} collaborator(s)` : ""}`,
        status: "Active",
        iconBg: "#e8f4ff",
        icon: "🚀",
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="px-10 pb-9 pt-24">
        {/* ── Top Bar ── */}
        <div className="flex items-start justify-between flex-wrap gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 h-9 w-52 shadow-sm">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="2.2" className="shrink-0">
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <StartNewProjectCard onClick={() => setShowModal(true)} />
          </div>
        </section>

        {/* ── Archive ── */}
        <section>
          <h2 className="text-base font-bold text-gray-800 mb-3">Archive</h2>
